                                ...
                                <div class="col-md-12">
                                    <form action="../app/controllers/usuarios/create.php" method="post">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="email" name="email" class="form-control" placeholder="Escriba aquí el correo del nuevo usuario..." required> <br>
                                            <?php
                                            if (isset($_SESSION['mensaje'])) {
                                                $mensaje = $_SESSION['mensaje'];

                                                // Muestra el mensaje (puedes diseñarlo como quieras)
                                                echo '<div class="alert alert-danger">' . $mensaje . '</div>';

                                                // Limpia el mensaje en la sesión para que no se muestre nuevamente
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", function (e) {
        ...
        if (!/^(?=.*[a-zA-Z0-9])(?=.*[^a-zA-Z0-9]).{8,}$/.test(passwordValue)) {
            e.preventDefault(); // Evita que se envíe el formulario

            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'La contraseña debe tener al menos 8 caracteres y al menos un carácter especial.',
            });
        }
    });
});
